...
Para simplificar esta formulação nós estamos considerando que os segmentos de video são requisitados a apenas um servidor fonte, bem como o serviço de video streaming já está em execução em seus respectivos servidores. Colocando as restrições abaixo:% An initial approach to the QoS-aware schedule is given by the following optimization problem:
...
A Restrição (5) estabelece as restrições de capacidade de banda disponível, que evita a atribuição de mais streaming de video do que a capacidade total disponível para cada nó de névoa. $bw_{ij}$ é o custo de banda de um nó $i$ ao nó $j$. 
...
% A topologia NSF (Figura 3(a)) e USA (Figura 3(b)) foram utilizadas. Na figura 3 mostra-se as distancias entre os nós.
...
É importante notar que, apesar das taxas de bits entre os mecanismos ABR serem semelhantes, as interrupções que eles podem gerar são significativamente diferentes, o que impacta diretamente no QoE dos usuários. Outro ponto a ser observado é a taxa de bits media entre diferentes níveis. No cenário com 20 usuários o nível 3 conseguiu atingir o que necessário para obter a maior representação de taxa bits, desta forma, operadores de telefonia podem buscar a implantação de caches durante horários de pico para prover a melhor experiência ao usuário.
...
\begin{figure*}[htb]
  ...
  \hfil \hspace{1cm}
    ...
    \label{fig:dmm-proposal}}
  \caption{Resultados da taxa de bits média e tempo total de interrupções para uma rede com 40 usuários, 20 usuários em cada AP.}
  \label{fig:dmm}
\end{figure*}
...
\begin{figure*}[htpb]
    \captionsetup{justification=centering}
    \centering
    \subfloat[Processo de decisão baseado na largura de banda.]
    {
        \noindent\makebox[\textwidth]{
    	    \includegraphics[width=0.9\paperwidth]{graphs/rate-User10-lvl1}
        }       
        \label{fig:lte-handover}
    }
...
\begin{figure*}[htb]
    ...
    \ContinuedFloat
        ...
        \subfloat[Processo de decisão baseado no buffer.]
        {
            \noindent\makebox[\textwidth]{
        	    \includegraphics[width=0.9\paperwidth]{graphs/buffer-User10-lvl1}
            }
            \label{fig:dmm-proposal}
        }
        \hfil
        \clearpage   
        \subfloat[Processo de decisão Híbrido.]
        {
            % \includegraphics[width=\textwidth]{graphs/hybrid-User10-lvl1}
            \noindent\makebox[\textwidth]{
        	    \includegraphics[width=0.9\paperwidth]{graphs/hybrid-User10-lvl1}
            }
            \label{fig:dmm-proposal}
        }
    }
...
%     \vspace{-2.5cm}
% 	{
%     % 	\includegraphics[width=\paperwidth]{graphs/rate-User10-lvl1}
%         \noindent\makebox[\textwidth]{
%         	\includegraphics[width=1.1\paperwidth]{graphs/rate-User10-lvl1}
%         }
%     	\vspace{-1.5cm}
%     	\caption{Cenários de distribuição de streaming de vídeo adaptável para nós multiníveis.}
%     	\label{fig:scenario-arch}
...
%     \vspace{-2.4cm}
% 	{
%     % 	\includegraphics[width=\paperwidth]{graphs/buffer-User10-lvl1}
%         \noindent\makebox[\textwidth]{
%         	\includegraphics[width=1.1\paperwidth]{graphs/buffer-User10-lvl1}
%         }
%     	\vspace{-1.4cm}
%     	\caption{Cenários de distribuição de streaming de vídeo adaptável para nós multiníveis.}
%     	\label{fig:scenario-arch}
...
% 	{
%         \noindent\makebox[\textwidth]{
%     	    \includegraphics[width=1.1\paperwidth]{graphs/hybrid-User10-lvl1}
%         }
% % 	\includegraphics[width=1.5\paperwidth,height=\textheight,keepaspectratio]{graphs/hybrid-User10-lvl1}
% 	    \vspace{-1.4cm}
% 	    \caption{Cenários de distribuição de streaming de vídeo adaptável para nós multiníveis.}
% 	    \label{fig:scenario-arch}
% 	}
...
% As Figuras 4(a) e 5(a) mostram a taxa de bits media de 5 usuários para o experimento de  20 usuários, respectivamente. No cenário com 20 usuários~(Figura 4(a)) as três heurísticas ABR apresentam um impacto semelhante. Enquanto que, intuitivamente, o provisionamento do video DASH entre os níveis 1 e 2 apresentam uma diferença estritamente superior, em contrapartida, o nível 3 apresenta uma disparidade significativa em relação aos outros níveis. Para o cenário com 40 usuários a taxa de bits média entre níveis apresentam diferenças semelhantes em ordem de grandeza. 